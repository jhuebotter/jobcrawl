...
| `kind` | TEXT | | The type of entity (e.g., "Academic", "Industry"). |
| `summary` | TEXT | | A brief description of the entity. |
| `website` | TEXT | | The primary URL for the entity. |
| `city` | TEXT | | The city where the entity is located. |
| `country` | TEXT | | The country where the entity is located. |
| `confidence` | REAL | | A score representing the confidence in the accuracy of the data. |
...
| Column | Type | Constraints | Description |
|---|---|---|---|
| `entity_id` | INTEGER | FOREIGN KEY (`entities.id`) | Foreign key to the `entities` table. |
...
| `entity_id` | INTEGER | FOREIGN KEY (`entities.id`) | Foreign key to the parent entity. |
| `name` | TEXT | NOT NULL | The person's full name. |
| `role` | TEXT | | The person's role or title. |
| `source_url` | TEXT | | The URL where the person's information was found. |
| `review_status` | TEXT | | Flag for associations with moderate/low confidence ('needs_review', 'confirmed'). |
...
| `entity_id` | INTEGER | FOREIGN KEY (`entities.id`) | The entity this source applies to. |
...
| `parameters` | TEXT | | JSON object of the run parameters (tags, cities, etc.). |
...
| `entity_id` | INTEGER | FOREIGN KEY (`entities.id`) | The entity that was edited. |
...
| Column | Type | Constraints | Description |
|---|---|---|---|
| `entity_id` | INTEGER | PRIMARY KEY, FOREIGN KEY (`entities.id`) | The entity that is starred. |
